...
const DESCRIBE: &str = r#"{
  ...
  "provenance_limitations": [
      ...
      "Correlated against OpenROAD check_antennas on a routed sky130 block (~2500 nets, same .odb), RE-MEASURED 2026-08-07 on a deterministic build against a freshly regenerated oracle. Against check_antennas as run on the design in 2026-05-26: 83 violations, 82 matched, 1 missed, 0 added, 80 of 82 values within 2%. Against check_antennas re-run today on a current build: 76 violations, 72 matched, 4 missed, 10 added, 71 of 72 within 2%. THE TWO OPENROAD RUNS DISAGREE WITH EACH OTHER on the same .odb -- 73 in common, 10 only the older build reports, 3 only the current one does -- so which answer is correct is not settled and this engine cannot be more correct than its reference. Every one of the 10 it reports that the current build does not is one the older build did report; net of the reference's own movement the standing difference is a single missed violation. Treat as a strong screen, not a sign-off gate: run check_antennas for sign-off, and if the two disagree check which build you are comparing against.",
