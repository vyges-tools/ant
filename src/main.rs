...
const DESCRIBE: &str = r#"{
  ...
  "provenance_limitations": [
      ...
      "NOT A SIGN-OFF GATE YET. Correlated against OpenROAD check_antennas on a routed sky130 block: 51 of 83 violations matched exactly, ~935 added that OpenROAD does not confirm, and 36 of 109 compared ratio values within 2%. Useful as a screen; run check_antennas to gate a tapeout.",
      "Metal is attributed per gate by walking the routed shape graph — the routing reachable from that gate over layers at or below the one being deposited. This replaced a per-net per-layer sum and cut false positives from ~2400 to ~935.",
      "Metal is measured as the exact union of a gate's connected rectangles, so overlap and abutment are counted once. Note this was NOT the source of the remaining over-reporting — replacing the raw sum with the union changed the measured result by a single violation.",
      "The numerator remains systematically high: of 109 records shared with OpenROAD none is below 0.9x, and of the 46 where the limit already agrees none is within 10%. Metal is still attributed to gates that OpenROAD does not attribute to them; the cause is connectivity rather than geometry.",
